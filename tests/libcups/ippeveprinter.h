...
ippeve_main(void *p1)			// I - Command-line arguments
{
  ...
  if (!directory[0])
  {
    ...
    LOG_INF("Making dir %s", directory);
    ...
    LOG_INF("Created dir %s", directory);
    ...
    cups_array_t *docformats)		// I - document-format-supported values
{
  ...
  LOG_INF("load_legacy_attributes: creating new attrs");
  attrs = ippNew();
